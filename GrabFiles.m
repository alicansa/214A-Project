...
for i=1:numel(EvalCleanList)
  EvalClean(i).name = EvalCleanList(i).name;
   EvalClean(i).data = wavread(strcat('2014/Eval/Clean/',EvalCleanList(i).name));
   EvalClean(i).Fs = 8000;
end
...
for i=1:numel(Eval10dBList)
   ...
   Eval10dB(i).data = wavread(strcat('2014/Eval/10dB/',Eval10dBList(i).name));
   Eval10dB(i).Fs = 8000;
end
...
for i=1:numel(Eval0dBList)
  ...
  Eval0dB(i).data = wavread(strcat('2014/Eval/0dB/',Eval0dBList(i).name));
  Eval0dB(i).Fs = 8000;
end
